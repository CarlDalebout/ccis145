...
class tankTorpedo( pygame.sprite.Sprite ):
    def __init__( self, screen, rank, owner, raw_speed, lifespan, maxrange, facing, position = (200,200) ):
        pygame.sprite.Sprite.__init__(self)
        if rank == 1:                                               # Standard
            Torp_Info = R1_TORPEDO                                  # Which Icon Set to use
        elif rank == 2:                                             # Unstable
            Torp_Info = R2_TORPEDO                                  # Which Icon Set to use
        else:
            return
        tankSFX = tankSound.gameSound()
        tankSFX.Play_Sound( "Energy Weapon-17" )
        
        self.def_facing     = 0.0                                   # set the direction the icon describs as 0
        self.facing         = facing
        self.frames         = Torp_Info[0]                          # set the frame number at which to wrap-around to 0
        self.cur_frame      = 0                                     # which frame are we currently on
        self.imglist        = []
        workImage           = tankImage.gameImage()
        for i in range(1, len(Torp_Info) ):
            workImage.image = workImage.Get_Image(Torp_Info[i])
        ...
        self.type           = "TORPEDO"                             # storing type of sprite
        ...
        if collisions:
            for impacted in collisions:
                ...
                if ( impacted == self.owner ):
                    debug("Safety Enabled!  Torpedo ignored friendly target.")
                    continue
                else:
                    one_hit = True
                    impacted.harm_tank( self.damage, DAM_TORPEDO, self.owner )
                #fi
            #rof
        #fi
        if one_hit:
