...
def get_scoreboard():
    ...
    insFont = pygame.font.SysFont('freesansbold', 36)
    scorelable = []
    score_list = file.readlines()
    tempLabel = insFont.render("Scores: ", 1, YELLOW )
    scorelable.append(tempLabel)
...
score_text = font.render(f"Killed Asteroids: {Asteroids_max-5}", True , BLUE)
question_text = font.render("Would you like to add your score to the board (y/n)", True, YELLOW)
...
while running:
    ...
    for i in range(len(insLabels)):
        screen.blit(insLabels[i], (50, 50+( 20*i )))

    screen.blit(score_text, (SCREEN_SIZE[0]/2-(score_text.get_width() + 5)/2, SCREEN_SIZE[1]/3))
    screen.blit(question_text,(SCREEN_SIZE[0]/2-(question_text.get_width() + 5)/2, SCREEN_SIZE[1]/3 + score_text.get_height()))

    for i in range(len(scores)):
        screen.blit(scores[i], (SCREEN_SIZE[0]/2 - 50, SCREEN_SIZE[1]/3 + 75+(20*i)))
...
file.write(f"{name}:  {Asteroids_max-5}\n")
file.close()
